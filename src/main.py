	...
	while (continuer):
		...
		if compteUtilisateur == None:#L'utilisateur n'est pas connecté
			...
			if compteUtilisateur.admin:
				...
				nombreChoisi = obtenirChoix(9)
			else:
				nombreChoisi = obtenirChoix(5)

				
def obtenirChoix(nombreMaxChoix : int) -> int:
	nombreRecu = ""
	while nombreRecu == "":
		nombreRecu = input("Entrez votre choix : ")
		try:
			nombreRecu = int(nombreRecu)
			if nombreRecu < 0 or nombreRecu > nombreMaxChoix:
